  ...
  useEffect(() => {
    const fetchUsers = async () => {
      if (selectedRestaurantUUID) {
        try {
          const fetchedUsers = await restaurantService.getUsersRestaurant(
            selectedRestaurantUUID
          );
          console.log('Fetched users:', fetchedUsers);
          setUsers(
            fetchedUsers.map((user) => ({
              value: user.uuid, // Use UUID for the value
              label: user.name || 'Unnamed User', // Use username for the label, fallback to 'Unnamed User'
            }))
          );
        } catch (error) {
    ...
    if (props.isVisible) {
      ...
      setValue('unit', props.ingredient?.unit ?? '');
    } else {
      reset();
    }
  }, [props.isVisible, reset, selectedRestaurantUUID, setValue]);
