...
export function FilterPopover({
  ...
  const [activeGroup, setActiveGroup] = useState<"tags" | "suppliers" | null>(
    null,
  );
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          size="icon"
          className={cn(selectedFilters.length > 0 && "border-solid")}
        >
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                >
                  ...
                  {selectedFilters.filter((f) => f.type === "supplier").length >
                    ...
                    <Badge variant="secondary" className="ml-auto">
                      {
                        selectedFilters.filter((f) => f.type === "supplier")
                          .length
                      }
                    </Badge>
                  )}
              ...
              <CommandGroup className="pt-0">
                ...
                {suppliers.map((supplier) => (
                  <CommandItem
                    ...
                    onSelect={() =>
                      toggleFilter({ type: "supplier", value: supplier })
                    }
                  >
                    <div
                      className={cn(
                        "mr-2 flex h-4 w-4 items-center justify-center rounded-sm border border-primary",
                        selectedFilters.some(
                          (f) => f.type === "supplier" && f.value === supplier,
                        )
                          ? "bg-primary text-primary-foreground"
                          : "opacity-50 [&_svg]:invisible",
                      )}
