...
export default function StocktakeSlider({
  stocktake,
  open,
  onOpenChange,
}: StocktakeSliderProps) {
        ...
        <div className="flex h-full divide-x divide-border">
          ...
          <div className="w-1/2 bg-gray-50/50 p-6">
            <div className="relative aspect-[3/2] bg-gray-100 rounded-lg overflow-hidden border shadow-sm">
              ...
              ) : stocktake.documents[selectedDocIndex]?.document_type ===
                "video" ? (
                <video
                ...
                <p className="text-sm text-muted-foreground">
                  {selectedDocIndex === -1
                    ...
                    : `${
                        ingredients.filter(
                          (ingredient) =>
                            ingredient.document_uuid ===
                            stocktake.documents[selectedDocIndex]
                              ?.document_uuid,
                        ).length
                      } items in document ${selectedDocIndex + 1}`}
                </p>
              </div>
                  ...
                  <TableBody>
                    {ingredients
                      .filter((ingredient) => {
                        ...
                        return (
                          selectedDoc &&
                          ingredient.document_uuid === selectedDoc.document_uuid
                        );
                      })
                      .map((ingredient) => (
                        <TableRow
                          key={`${ingredient.ingredient_uuid}-${ingredient.document_uuid}`}
                        >
